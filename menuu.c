...
int main(){
    ...
    char nome_avaliacao[100];
    int *avaliacao = malloc(1*sizeof(int));
    ...
    arq = fopen("streaming_db.csv", "r");
        ...
        avaliacao = realloc(avaliacao,sizeof(int)*contadorDeLinhasArquivo);
    ...
    while(energia == 1){
        ...
        switch(opcao){
                ...
                funcao_LimpaTela();
                    ...
                    for(int  i = 0; i < contadorDeLinhasArquivo; i++){
                        if(series[i].qnt_temporadas > 0){
                            ...
                            if (avaliacao[i]>=0 && avaliacao<=10){
                                printf("Nota do usuário: %d\n", avaliacao[i]);
                            }
                        } else {
                            i++;
                        }
            ...
            case 6:
            //Permite ao usuáro avaliar uma série
                funcao_LimpaTela();
                    printf("\n\033[47m\033[1;32m##########--##########\033[47m\033[1;32m\n");
                    printf("\n\033[47m\033[1;32Insira o nome da série:\033[47m\033[1;32m\n");//Obtém o nome da série
                    Get_String(nome_avaliacao, 50);

                    for(int i=0; i<contadorDeLinhasArquivo;i++){
                        if (strcmp(series[i].titulo,nome_avaliacao)==0){
                            achado = i;
                        }//Caso exista, achado = posição
                    }//For para verificar se a série existe
                    if (achado!=0){//Caso tenha achado a série na base de dados
                        printf("A série consta na nossa base de dados. Como deseja avaliá-la?(Utilize números inteiros de 1 a 10).\n");
                        scanf("%d",&avaliacao[achado]);
                        setbuf(stdin,NULL);
                        printf("A série foi avaliada. Obrigada pela sua opinião!\n");
                    }//if
                    else{//Caso não encontre a série
                        printf("A série não está nos nossos dados! Tente adicioná-la você mesmo antes.\n");
                    }//else
                    achado = 0;//Reseta a var achado para outras pesquisas
